  ...
  ({ wait = 500, column, className, ...restProps }, forwardedRef) => {
    ...
    useEffect(() => {
      const filtered =
        searchVal === ""
          ? sortedUniqueValues
          : sortedUniqueValues.filter((item: string) => item.toLowerCase().includes(searchVal.toLowerCase()));
          ...
          <>
            <div className="flex items-center">
              <Combobox.Input
                {...restProps}
                ref={forwardedRef}
                className="w-full h-10 focus:ring-0 text-sm px-8 outline-none placeholder:text-gray-400 border-b-gray-200 border-x-transparent border-t-transparent focus:border-x-transparent focus:border-t-transparent focus:border-gray-200"
                value={searchInput}
                onChange={(e) => {
              ...
              {!isSearching ? (
                ...
                <svg
                  ...
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-5 h-5 left-2 absolute text-gray-500 animate-spin"
                >
                  <path
                    opacity="0.2"
                    fillRule="evenodd"
              ...
              <button
                className="h-4 w-4 text-white bg-gray-400 absolute right-2 flex items-center justify-center rounded-full"
                onClick={() => {
            ...
            {open ? (
              <>
                <Combobox.Options as="ul" className="max-h-40 overflow-y-auto overflow-x-hidden border-y">
                ...
                <footer className="flex justify-center text-xs text-gray-500 border-t p-2 font-medium">
